  ...
  <div v-if="show">
    <el-form label-position="top">
      ...
      <el-form-item label="大小" v-if="Object.keys(data.options).indexOf('size')>=0">
        宽度：
        <el-input style="width: 80px;" type="number" v-model.number="data.options.size.width"></el-input>
        高度：
        <el-input style="width: 80px;" type="number" v-model.number="data.options.size.height"></el-input>
      </el-form-item>
      ...
      <template v-if="data.type != 'grid'">
        <el-form-item label="属性" v-if="data.options.hasOwnProperty('isFlowProp')">
          <el-checkbox v-model="data.options.isFlowProp" >是否为流程属性
          </el-checkbox>
        </el-form-item>
      ...
      <template v-if="data.type=='table'">
        ...
        <el-form-item label="表头">
          <el-input type="text" v-model="labelValue"></el-input>
        </el-form-item>
        <el-form-item label="表格属性值">
          <el-input type="text" v-model="propValue"></el-input>
        </el-form-item>
        <el-form-item label="单元格宽度">
          <el-input type="number" v-model.number="widthValue"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="success" @click="addProp">添加属性</el-button>
          <el-button type="danger" @click="delProp">删除属性</el-button>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      //添加表头
      addProp() {
        if (this.propValue == null || this.labelValue == null) {
          this.$message.warning("请输入表头和属性值！")
        } else {
          let hasProp = false;
          if (this.data.options.tablePropData.length > 0) {
            this.data.options.tablePropData.some(item => {
              if (item.prop == this.propValue) {
                hasProp = true;
                return true;
              }
            });
            if (!hasProp) {
              ...
              this.data.options.tablePropData.push(obj);
              this.propBack();
            } else {
              this.$message.warning("已存在相同属性，请修改属性值")
            }
          } else {
            let obj = {
              label: this.labelValue,
      ...
      //删除表头
      delProp() {
        this.data.options.tablePropData.splice(-1)
      },
      //初始化
      propBack() {
